...
void GameOfLife(tpLattice *mLattice){
    ...
    if ( mLattice->world > 1){
      if ((mLattice->rank > 0) && (mLattice->rank < mLattice->world - 1)){
        ...
        MPI_Request mRequestInA, 
                    mRequestInB,
                    mRequestOutA, 
                    mRequestOutB;;

  
        MPI_Isend(&(mLattice->buff0[(mLattice->width * (mLattice->height - 2))]), mLattice->width, MPI_BYTE, mLattice->rank+1, 0, MPI_COMM_WORLD, &mRequestOutA);
        MPI_Irecv(&(mLattice->buff0[(mLattice->width * (mLattice->height - 1))]), mLattice->width, MPI_BYTE, mLattice->rank+1, 0, MPI_COMM_WORLD, &mRequestInA);

        MPI_Isend(&(mLattice->buff0[mLattice->width]), mLattice->width, MPI_BYTE, mLattice->rank-1, 0, MPI_COMM_WORLD, &mRequestOutB);
        MPI_Irecv(mLattice->buff0, mLattice->width, MPI_BYTE, mLattice->rank-1, 0, MPI_COMM_WORLD, &mRequestInB);

        MPI_Status mStatus;
        MPI_Wait(&mRequestInA, &mStatus);
        MPI_Wait(&mRequestInB, &mStatus);
        MPI_Wait(&mRequestOutA, &mStatus);
        MPI_Wait(&mRequestOutB, &mStatus);

      }else if (mLattice->rank == 0){

        MPI_Request mRequestA, mRequestB;
        MPI_Isend(&(mLattice->buff0[(mLattice->width * (mLattice->height - 2))]), mLattice->width, MPI_BYTE, mLattice->rank+1, 0, MPI_COMM_WORLD, &mRequestA);
        MPI_Irecv(&(mLattice->buff0[(mLattice->width * (mLattice->height - 1))]), mLattice->width, MPI_BYTE, mLattice->rank+1, 0, MPI_COMM_WORLD, &mRequestB);

        MPI_Status mStatus;
        MPI_Wait(&mRequestA, &mStatus);
        MPI_Wait(&mRequestB, &mStatus);
      }else if (mLattice->rank == mLattice->world - 1){
        //pega apenas a parte de cima

        MPI_Request mRequestA, mRequestB;
        MPI_Isend(&(mLattice->buff0[mLattice->width]), mLattice->width, MPI_BYTE, mLattice->rank-1, 0, MPI_COMM_WORLD, &mRequestA);
        MPI_Irecv(mLattice->buff0, mLattice->width, MPI_BYTE, mLattice->rank-1, 0, MPI_COMM_WORLD, &mRequestB);
